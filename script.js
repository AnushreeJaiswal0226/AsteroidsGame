...
var fxExplode = new Sound("explode.mp3");
var fxHit = new Sound("hit.mp3", 5);
var fxLaser = new Sound("laser.mp3", 5, 0.5);
var fxThrust = new Sound("thrust.mp3");
...
if(scoreStr == null){
    ...
    highscoreEl.innerHTML = parseInt(scoreStr);
    bigHighScoreEl.innerHTML = parseInt(scoreStr);
}
...
function drawShip(x, y, a, colour = "white") {
    ...
    c.arc((x + 4 / 3 * ship.r * Math.cos(a)), (y - 4 / 3 * ship.r * Math.sin(a)), 4, 0, 2 * Math.PI, false);
    c.fillStyle = 'red';
    c.stroke();
    c.fill();
}
...
function update() {
    ...
    if (ship.thrusting && !ship.dead) {
        ...
        if (!exploding && blinkOn) {
            ...
            c.lineWidth = SHIP_SIZE / 10;
            c.beginPath();
            c.moveTo( // rear left
                ship.x - ship.r * (2 / 3 * Math.cos(ship.a) + 0.5 * Math.sin(ship.a)),
                ship.y + ship.r * (2 / 3 * Math.sin(ship.a) - 0.5 * Math.cos(ship.a))
            );
            c.lineTo( // rear centre (behind the ship)
    ...
    if (!exploding) {
        ...
        c.fillStyle = "darkred";
        c.beginPath();
        c.arc(ship.x, ship.y, ship.r * 1.7, 0, Math.PI * 2, false);
        c.fill();
        c.fillStyle = "red";
        c.beginPath();
        c.arc(ship.x, ship.y, ship.r * 1.4, 0, Math.PI * 2, false);
        c.fill();
        c.fillStyle = "orange";
        c.beginPath();
        c.arc(ship.x, ship.y, ship.r * 1.1, 0, Math.PI * 2, false);
        c.fill();
        c.fillStyle = "yellow";
        c.beginPath();
        c.arc(ship.x, ship.y, ship.r * 0.8, 0, Math.PI * 2, false);
        c.fill();
        c.fillStyle = "white";
        c.beginPath();
        c.arc(ship.x, ship.y, ship.r * 0.5, 0, Math.PI * 2, false);
        c.fill();
    }
    ...
    for (var i = 0; i < ship.lasers.length; i++) {
        if (ship.lasers[i].explodeTime == 0) {
            c.fillStyle = "salmon";
            c.beginPath();
            c.arc(ship.lasers[i].x, ship.lasers[i].y, SHIP_SIZE / 15, 0, Math.PI * 2, false);
            c.fill();
        } else {
            // draw the eplosion
            c.fillStyle = "orangered";
            c.beginPath();
            c.arc(ship.lasers[i].x, ship.lasers[i].y, ship.r * 0.75, 0, Math.PI * 2, false);
            c.fill();
            c.fillStyle = "salmon";
            c.beginPath();
            c.arc(ship.lasers[i].x, ship.lasers[i].y, ship.r * 0.5, 0, Math.PI * 2, false);
            c.fill();
            c.fillStyle = "pink";
            c.beginPath();
            c.arc(ship.lasers[i].x, ship.lasers[i].y, ship.r * 0.25, 0, Math.PI * 2, false);
            c.fill();
        }
    }
    ...
    if (textAlpha >= 0) {
        ...
        c.font = "small-caps " + TEXT_SIZE + "px Orbitron, sans-serif";
        c.fillText(text, canvas.width / 2, canvas.height * 0.75);
...
//starting the game from the start button
startGameBtn.addEventListener('click', () => {
    ...
    //restarting new game
    if(startGameBtn.innerHTML == "R e s t a r t") location.reload();
})